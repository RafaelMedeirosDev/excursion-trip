...
- **Entidades implementadas:** `Organization` (só `create`, `POST /organizations`, checagem de duplicidade de CNPJ), `User` (só `create`, `POST /users`, senha com hash bcrypt, checagem de duplicidade de email global + cpf por organização), `Customer` (só `create`, `POST /customers`, checagem de duplicidade de cpf por organização, sem senha/role), `Event` (só `create`, `POST /events`, sem checagem de duplicidade — `Event` não tem nenhum campo `@unique` no schema) e `Excursion` (só `create`, `POST /excursions`, valida que o `eventId` do body pertence à organização do token). Todas as outras ainda não têm Controller/Service/Domain/External — não implemente uma entidade especulativamente, só quando pedido.
...
- **Rotas protegidas** (`@UseGuards(JwtAuthGuard, RolesGuard)`, Parte 2 da skill `auth`) — só `POST /auth/login` continua público:
  - `POST /organizations`, `POST /users`, `POST /events` e `POST /excursions`: `@Roles(Role.ADM)` — só admin (ações administrativas/de planejamento). `POST /organizations` não é signup público: um ADM já autenticado cria organizações novas manualmente quando precisa, não existe auto-cadastro.
  - `POST /customers`: sem `@Roles` — qualquer autenticado (`ADM` ou `EMPLOYEE`) pode cadastrar cliente, é tarefa operacional do dia a dia.
  - `POST /users`, `POST /customers`, `POST /events` e `POST /excursions` usam `@CurrentUser().organizationId` (do JWT), não confiam em `organizationId` vindo do body — o campo nem existe nesses DTOs. `POST /excursions` também usa `@CurrentUser().sub` como `userId` (quem criou).
