...
- **Login (JWT) implementado**: `POST /auth/login` (`200`, devolve `{ accessToken }`), seguindo a skill `.claude/skills/auth` Parte 1. `JwtStrategy`, `JwtAuthGuard`, `RolesGuard`, decorators `Roles`/`CurrentUser` já existem — não recriar. **Nenhuma rota ainda está protegida** (`POST /organizations` e `POST /users` continuam abertas) — aplicar `@UseGuards`/`@Roles` (Parte 2 da skill) é um passo futuro, ainda não feito.
