...
- **Rotas protegidas** (`@UseGuards(JwtAuthGuard, RolesGuard)`, Parte 2 da skill `auth`) — só `POST /auth/login` continua público:
  - `POST /organizations`, `POST /users` e `POST /events`: `@Roles(Role.ADM)` — só admin (ações administrativas/de planejamento). `POST /organizations` não é signup público: um ADM já autenticado cria organizações novas manualmente quando precisa, não existe auto-cadastro.
  - `POST /customers`: sem `@Roles` — qualquer autenticado (`ADM` ou `EMPLOYEE`) pode cadastrar cliente, é tarefa operacional do dia a dia.
  - `POST /users`, `POST /customers` e `POST /events` usam `@CurrentUser().organizationId` (do JWT), não confiam em `organizationId` vindo do body — o campo nem existe nesses DTOs.
- **Setup compartilhado já existe** (criado uma única vez, não recriar): `src/external/repositories/remote/PrismaRemoteRepository.ts` (wrapper do `PrismaClient`), `src/shared/erros/base/AlreadyExistsError.ts` (`ConflictException`) e `src/shared/erros/base/UnauthorizedError.ts` (`UnauthorizedException`) — reaproveitáveis por qualquer entidade. `@nestjs/config` e `@nestjs/swagger` (`^7`, não `^11` — o projeto está no Nest 10) já são dependências do projeto.
