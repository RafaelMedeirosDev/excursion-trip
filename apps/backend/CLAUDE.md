...
- **Entidades implementadas:** `Organization` (só `create`, `POST /organizations`, checagem de duplicidade de CNPJ) e `User` (só `create`, `POST /users`, senha com hash bcrypt, checagem de duplicidade de email global + cpf por organização). Todas as outras ainda não têm Controller/Service/Domain/External — não implemente uma entidade especulativamente, só quando pedido.
- **`User.email` é único globalmente** (não mais por organização — mudou na feature de login, migration `20260715010000_make_email_globally_unique`). `User.cpf` continua único só por organização.
...
- **`POST /organizations` e `POST /users` estão protegidas** (`@UseGuards(JwtAuthGuard, RolesGuard)` + `@Roles(Role.ADM)`, Parte 2 da skill `auth`) — só `ADM` autenticado consegue chamar. `POST /organizations` não é signup público: o próprio usuário (via um ADM já existente) cria organizações novas quando precisa, não existe fluxo de auto-cadastro. `POST /users` usa `@CurrentUser().organizationId` (do JWT), não confia mais em `organizationId` vindo do body — o campo foi removido do `CreateUserDTO`. Só `POST /auth/login` continua público (sem guards).
