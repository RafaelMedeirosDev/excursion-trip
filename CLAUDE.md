...
- **Todas as 11 entidades do schema têm `create` implementado** (`Organization`, `User`, `Customer`, `Event`, `Excursion`, `Supplier`, `VehicleBooking`, `Expense`, `BoardingPoint`, `Reservation` e `Payment` — domínio completo). `User` e `Customer` também têm `list` (`GET /users`, `GET /customers`) — ver `apps/backend/CLAUDE.md` pro padrão a seguir em próximos `list`. Não implemente `list`/`update`/`delete` especulativamente pras demais entidades, espere pedido explícito.
