...
- **`Organization`, `User`, `Customer` e `Event` (create) implementadas** (`POST /organizations`, `POST /users`, `POST /customers`, `POST /events`); as demais entidades ainda não — não implemente uma especulativamente, espere pedido explícito.
- **Login (JWT) implementado e rotas protegidas**: `POST /auth/login` é a única rota pública. `POST /organizations`/`POST /users`/`POST /events` exigem `ADM` autenticado; `POST /customers` aceita qualquer autenticado (`ADM` ou `EMPLOYEE`). Não existe signup público — organizações novas são criadas manualmente por um ADM já existente.
